...
(313 verbs with transitivity and sense pins) appears in the
SKILL file, not here. Rules, pitfalls, examples, and the
evidential system appear in the SKILL file.

Load this file alongside the SKILL file for conversion and
validation tasks. For generation tasks, the SKILL file alone
suffices.
...
These nouns carry two approved senses. Context determines
which applies. The sense pin in each category section
indicates which sense belongs there.

| Noun | Sense 1 | Sense 2 |
|---|---|---|
| `cell` | compartment in a structure | smallest unit of a living organism |
| `current` | electric charge flow per unit time | directed fluid flow |
| `element` | substance with one atom type | single part in a structure |
| `field` | region where force acts | bounded area of ground |
| `lead` | metallic element, atomic number 82 | electrical conductor wire |
| `plate` | flat rigid sheet | tectonic crust section |
| `crystal` | ordered-lattice solid | shaped optical solid |

## Dropped Nouns (do not use)

| Dropped | Use instead |
|---|---|
| `vessel` | `container` (holding); `transportation` + `device` (vehicle) |
| `apparatus` | `device`, `instrument` |
| `machine` | `device`, `engine` |
| `craft` | `transportation` + `device` |
| `fitting` | `connector`, `joint` |
| `fixture` | `mount`, `bracket` |
| `hood` | `cover`, `shield` |
| `lining` | `layer` |
| `mat` | `pad` |
| `trim` | `strip` |
